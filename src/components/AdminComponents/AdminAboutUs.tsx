...
const AdminAboutUs = () => {
  ...
  const id = "sdfg5s4fd5g-asdfasdf-5465";
  const popupRef = useRef<HTMLDivElement>(null);

  const [actualizeData, setActualizeData] = useState<AboutUsPage>({
    id: "",
    text1: "",
    foto1: "",
    text2: "",
  ...
  const getData = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/admin/aboutus/getaboutuspage/${id}`,
        {
          method: "GET",
          headers: {
